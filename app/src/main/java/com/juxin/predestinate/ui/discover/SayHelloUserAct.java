...
public class SayHelloUserAct extends BaseActivity implements AdapterView.OnItemClickListener,
    ...
    private void initData() {
        ...
        //在编辑中保留之前已选择的选项
        if (isGone && delList.size() > 0) {
            exListView.clearAllChooseChecked();
            for (BaseMessage msg : delList) {
                for (int i = 0; i < adapter.getCount(); i++) {
                    BaseMessage baseMessage = adapter.getItem(i);
                    if (msg.getLWhisperID() == baseMessage.getLWhisperID()) {
                        exListView.selectChooseView(i, true);
                        break;
                    }
                }
            }
    ...
    private void onTitleRight() {
        ...
        if (!ModuleMgr.getCenterMgr().getMyInfo().isMan()) {
            title_right = LayoutInflater.from(getActivity()).inflate(R.layout.f1_mail_title_right_autoreply, null);
            final BadgeView badge_auto_reply = (BadgeView) title_right.findViewById(R.id.badge_auto_reply);
            badge_auto_reply.setPoint();

            badge_auto_reply.setVisibility(
                    PSP.getInstance().getBoolean(
                            ModuleMgr.getCommonMgr().getPrivateKey(FinalKey.ISSHOWAUTOREPLYPOINT_SAYHELLO), true)
                            ? View.VISIBLE : View.GONE);

            title_right.findViewById(R.id.iv_auto_reply).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    PSP.getInstance().put(ModuleMgr.getCommonMgr().getPrivateKey(FinalKey.ISSHOWAUTOREPLYPOINT_SAYHELLO), false);
                    badge_auto_reply.setVisibility(View.GONE);
                    UIShow.showWomanAutoReply(SayHelloUserAct.this);
                    Statistics.userBehavior(SendPoint.page_peopleofsayhi_autoreply);
                }
            });
        } else {
            title_right = LayoutInflater.from(getActivity()).inflate(R.layout.f1_mail_title_right, null);
