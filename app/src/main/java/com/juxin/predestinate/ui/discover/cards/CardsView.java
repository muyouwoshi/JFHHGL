        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (isTouchTopView(ev)){
                    getParent().requestDisallowInterceptTouchEvent(true); //使父控件不进行事件的拦截
                }
                break;
...
//        mScrolling = true;
    ...
    private void moveTopView(int deltaX, int deltaY) {
        ...
        if (topView != null) {
            ...
            int goalDegree = (int) (((double) (topView.getX() - initLeft) / (double) mWidth) * 7);
            if (goalDegree >= 10) {
                goalDegree = 10;
            } else if (goalDegree <= -10) {
                goalDegree = -10;
            }
            topView.setRotation(goalDegree);
     ...
     *
     * @param type {@link com.juxin.predestinate.ui.discover.cards.CardsView.SlideType}
     */
